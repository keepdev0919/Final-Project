...
struct TasteDiscoveryView: View {
    ...
    /// 그래서 "3박4일"은 4를 보낸다 (박 + 1 = 일).
    ///
    /// 백엔드는 고른 일수와 **정확히 같은** 코스만 준다. 사용자가 이미 자기
    /// 여행에 맞는 일수를 골랐기 때문이다.
    ///
    /// 예외는 마지막 선택지뿐이다 — "3박4일 이상"은 라벨대로 4일 이상을 다 담는다.
    /// 백엔드가 이 값(4)을 열린 구간의 시작으로 안다(`OPEN_ENDED_FROM`).
    private let durationOptions: [(days: Int, label: String)] = [
        (1, "당일치기"), (2, "1박2일"), (3, "2박3일"), (4, "3박4일 이상"),
    ]
