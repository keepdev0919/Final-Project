...
### 방향 2차 재정립: 정보→경험, 설화 강등, 현장 반응형 대화 엔진

- **본질 재정의:** "정보를 파는 게 아니라 경험을 판다." 설화·민담을 목적에서 **'데이터1'(재료 후보 하나)로 강등.**
- **유료기능 축 반전:** "도착 시 설화 RAG 질문 AI" 폐기 방향 → **"현장에서 눈앞의 것에 반응하는 대화 가이드."** RAG는 옆길 질문 보조로 강등.
- **엔진:** 장소=관찰 지점 경로, 각 지점=반응형 대화 루프. 아키텍처="레일 위 자유 대화"(비트 명세 콘텐츠 템플릿 + 재사용 런타임 엔진). 앵커링=GPS 웨이포인트+자기보고(카메라 백로그).
- **전략:** 성산일출봉 1개 압도적 완성→검증→템플릿화→확장(양산 지양). 게임화=수단으로 격하.
- **문서:** `docs/기획/방향성_재정립_2026-07-20.md` 신설(방향 SSoT). 구방향 문서 지층 정리(배너/아카이브). deep-research 리서치 착수.
