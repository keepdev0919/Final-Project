...
데이터 파이프라인은 제주도청 공공 API에서 수집한 설화·민담을 지명 Geocoding으로
좌표화하여 ChromaDB에 벡터화하고, 한국관광공사 KTO 분류와 키워드 규칙을
결합한 3단계 필터로 Visit Jeju 장소 3{,}284개에서 관광지 820개를 선별하여
설화 매핑 신뢰성을 약 77\% 개선하였다. 최종 보고 시점 기준으로 홈 화면,
...
(1,749)    courses    GPT-4o /  Maps SDK   Auth +
chunks     9,134 +    gpt-      / KTO      Firestore
           place_rev  image-1   TourAPI    (iOS SDK)
\end{verbatim}
\end{footnotesize}
...
체험/마을시설/종교) 820개만 설화 매핑 대상으로 한정하였다. 표 1은
...
장소-설화 매핑 건수 & $\sim$134{,}000 & \textbf{$\sim$31{,}000} \\
GPS 보조 매핑 노이즈 & $\sim$112{,}000 & $\sim$24{,}000 \\
신뢰성 개선 & --- & \textbf{$-77\%$} \\
\bottomrule
\end{tabular}
\end{table}
...
GPS 좌표가 확보된 항목(269건 중 235건, 87\%)은 OpenAI Text Embedding\,[6]을
이용해 청크로 분할하여 ChromaDB에 벡터화하고(1{,}749 청크), 좌표 미확보
항목도 텍스트 검색용으로 ChromaDB에 함께 적재하였다.
...
\subsection{코스 추천: 2단계 LLM 워크플로우와 랜덤 풀}

응답 지연을 최소화하기 위해 코스 추천을 두 단계의 함수 파이프라인으로
분리하였다. 첫 번째 단계(\texttt{POST /course/list})에서는 \texttt{course\_list\_agent}
모듈이 사용자의 취향 카테고리 점수가 0점을 초과하는 코스 전체를 SQLite에서
후보 풀로 구성한 뒤, 그 안에서 \textbf{매번 무작위로 3개를 뽑아} GPT-4o가
여행 스타일에 적합한 순서로 정렬·반환한다. 본 단계는 도구 자율 호출이
없는 단일 LLM 호출 기반 워크플로우로, ReAct 패턴은 사용하지 않는다. 두
번째 단계(\texttt{POST /course/detail})에서는 결정론적 함수가 선택된 코스의
모든 장소에 대해 haversine 거리로 반경 3km 이내 설화를 매핑하고, GPT-4o가
설화 취향과 장소 정보를 바탕으로 2문장 여행 내러티브를 \texttt{with\_structured\_output}
스키마로 생성한다.

\textbf{랜덤 풀 방식의 도입 배경.}\quad 초기 구현에서는 점수 상위 3개를
결정론적으로 반환하여 동일 입력에 동일 결과가 나오는 문제가 있었다. 시연
시 매번 같은 코스가 노출되어 다양성이 부족했고, 실사용 측면에서도 같은
취향 사용자가 재방문 시 새로운 추천을 받지 못하는 한계가 있었다. 카테고리
매칭이 전혀 안 되는 코스는 풀에서 배제하므로 추천의 의미는 유지하면서,
같은 취향으로 다시 진입해도 매번 다른 조합을 보게 된다. 매칭이 0건이면
빈 결과를 반환하여 클라이언트가 ``조건에 맞는 코스 없음'' 화면으로 자연스럽게
유도된다(점수 무관 fallback 제거).
...
설화·민담 원본 & 505건 \\
ChromaDB 벡터 청크 & 1{,}749개 \\
GPS 매핑된 설화 & \textbf{235 / 269 (87\%)} \\
장소-설화 매핑 (필터 전 → 후) & 134k → 31k ($-77\%$) \\
...
본 연구는 다음 한계를 가진다. 첫째, 설화 GPS 매핑이 87\%(235/269)에 그쳐
약 13\%의 설화는 좌표가 없어 텍스트 검색으로만 활용된다. 채록 메타데이터의
조사 장소 정보가 불완전한 경우가 주된 원인이다. 둘째, 5종 페르소나에 대한
사용자 평가 데이터가 부재하여, 어느 페르소나가 어떤 사용자 군에 가장
효과적인지에 대한 정량적 근거가 아직 없다. 셋째, 실기기 현장 GPS 테스트가
시뮬레이션 위주로 진행되어, 제주 현지의 다중 경로·도심 협곡 환경에서의
지오펜스 신뢰성 검증이 충분하지 않다.
...
3{,}284개 장소를 820개 관광지로 한정하여 설화 매핑 신뢰성을 77\% 개선하였고,
설화·민담 505건을 ChromaDB 1{,}749 청크로 벡터화하였으며, 설화 GPS 매핑은
87\%에 도달하였다.
