...
좌표화하여 ChromaDB에 벡터화하고(1{,}749 청크 적재, GPS 부착 228건/961 청크만 동행자
채팅 검색 대상으로 한정), 한국관광공사 KTO 분류와 키워드 규칙을 결합한 3단계 필터로
Visit Jeju 장소 3{,}284개에서 관광지 820개를 선별하였다. 또한 장소-설화 사전 매핑
30{,}947건 중 GPS 보조 매핑을 제외한 6{,}726건만 코스 추천에 사용하여, 두 검색 경로
모두에 일관된 \emph{장소 정합성 원칙}을 적용하였다(원본 매핑 134k 대비 노이즈
$-77\%$). 최종 보고 시점 기준으로 홈 화면,
...
(1,749 적재  courses    GPT-4o /  Maps SDK   Auth +
 /961 검색)  9,134 +    gpt-      / KTO      Firestore
 chunks      place_rev  image-1   TourAPI    (iOS SDK)
\end{verbatim}
\end{footnotesize}
...
1{,}749 청크로 벡터화하였고, 그중 GPS 좌표가 확보된 항목(505건 중 228건, 약 45\%)은
별도로 \texttt{has\_gps=True} 메타데이터를 부여하여 961 청크가 동행자 채팅의
의미 검색 대상이 된다. 좌표 미확보 항목도 ChromaDB에는 함께 적재되어 향후
다른 기능에서 재활용 가능하도록 보존하였다.
...
ChromaDB 의미 검색
(동행자 채팅) &
메타데이터 \texttt{has\_gps} (bool) /
쿼리 \texttt{where=\{has\_gps: True\}} \\
\addlinespace[2pt]
사전 매핑 테이블
(코스 추천) &
...
GPS 좌표가 부착된 228건(961 청크)에만 \texttt{has\_gps=True} 메타데이터를 부여하고
동행자 채팅 검색 쿼리에 \texttt{where} 필터를 적용해 \emph{GPS 부착 청크만} 검색
...
GPS 부착 청크 (동행자 채팅 검색) & \textbf{961개} \\
GPS 좌표 부착 설화·민담 & \textbf{228 / 505 ($\sim$45\%)} \\
...
본 연구는 다음 한계를 가진다. 첫째, 설화·민담 GPS 부착률이 약 45\%(228/505)에
그쳐, 좌표가 없는 277건은 동행자 채팅 의미 검색에서 제외되고 향후 다른 기능을
위한 보존용으로만 ChromaDB에 적재되어 있다. 설화 본문 내 지명 표현의 모호성과
민담 채록 메타데이터의 조사 장소 정보가 불완전한 것이 주된 원인이다. 둘째, 5종 페르소나에 대한
...
사용한다. 설화·민담 505건은 ChromaDB에 1{,}749 청크로 벡터화하였고, GPS 부착 228건/961 청크가
동행자 채팅의 의미 검색 대상이 된다.
