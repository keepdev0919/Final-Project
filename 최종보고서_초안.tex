...
연결되는 iOS 모바일 서비스를 구현한다. \textbf{2단계 결정론적 LLM 워크플로우}가
지역·스타일·기간 입력을 받아 코스 후보 풀에서 무작위 3건을
...
데이터 파이프라인은 제주도청 공공 API에서 수집한 설화·민담 505건을 OpenAI
임베딩으로 벡터화하되, 비짓제주 관광지와 직접 매핑된 397 설화·민담의 \textbf{1{,}262
청크}만 동행자 채팅의 의미 검색 대상으로 한정하였다. 한국관광공사 KTO 분류와 키워드 규칙을 결합한
...
RAG, 페르소나 LLM, ChromaDB, \textbf{결정론적 워크플로우}, iOS Swift, 위치 기반 서비스,
...
제주관광공사 Visit Jeju의 실제 관광객 여행 일정 데이터를 기반으로 \textbf{결정론적
함수 파이프라인}이 설화·민담이 풍부한 코스를 추천하고, 여행
...
\subsection{RAG 기반 LLM 워크플로우와 응답 지연 통제}

대화형 시스템에서 응답 지연은 사용자 이탈에 직접적인 영향을 미친다. Yao et
al.(2023)이 제안한 ReAct(Reasoning + Acting) 패턴은\,[8] LLM이 추론과 도구
호출을 반복하여 복잡한 작업을 해결하지만, 호출 라운드가 누적될수록 응답
지연이 증가하며 비용과 환각 위험도 함께 커진다. 본 연구는 응답 속도가 사용자
경험의 핵심인 영역에서는 ReAct를 의도적으로 배제하고, 사전 정제된 데이터와
결정론적 함수의 조합 위에 단일 LLM 호출을 얹는 방식을 채택하였다. LLM 호출은
동행자 채팅·일지 생성·민화 이미지 프롬프트 생성 등 자연어 창의성이 필수인
지점에 한정하여 사용한다.

\subsection{페르소나 기반 대화 인터페이스}

Park et al.(2023)은 ``Generative Agents''에서 LLM이 기억·반영·계획 모듈을
통해 일관된 개성을 유지하며 상호작용하는 시뮬레이션을 제시하였다\,[10].
...
다룬다\,[11]. 또한 함수 호출(function calling) 및 JSON 스키마 응답
기능은 LLM의 출력을 다운스트림 시스템이 안정적으로 소비할 수 있는 구조화된
형태로 강제한다. 이미지 생성 모델인 gpt-image-1\,[12]은 텍스트 프롬프트로
...
(1,262       courses    GPT-4o /  Maps SDK   Auth +
 청크,       9,134 +    gpt-      / KTO      Firestore
 397 매핑)   place_rev  image-1   TourAPI    (iOS SDK)
\end{verbatim}
\end{footnotesize}
...
\item \textbf{KTO 정식 관광지 매칭}: 한국관광공사 TourAPI\,[9]의 제주
...
벡터화하였고, 그중 비짓제주 관광지와 직접 매핑된 항목(505건 중 397건,
약 78\%)에만 \texttt{is\_mapped=True} 메타데이터를 부여하여 동행자 채팅의 의미
검색 대상으로 한정하였다. 매핑된 397 설화·민담은 \textbf{1{,}262 청크}로 분할
저장되어 의미 검색의 대상 공간을 이룬다. 매핑이 발생하지 않은 108건(본문이 매우
짧거나 매칭 가능 지명이 없는 경우)도 ChromaDB에 함께 저장되어 향후 자유 발화
챗봇 등 다른 기능에서 재활용 가능하도록 보존하였다.

\textbf{[예시] 한 건의 설화가 벡터 DB에 등록되는 과정.}\quad 본 가공 파이프라인이
한 건의 설화에 적용되는 모습을 살펴본다. 설화 ``각시당본풀이''(C\_M\_001)의
원본은 HTML 정제 후 1{,}162자다. \texttt{chunk\_size=900}, \texttt{overlap=120}의
슬라이딩 윈도우로 분할하면 청크 0(0--900자, 길이 900)과 청크 1(780--1{,}162자,
길이 382) 두 개가 생성된다. 두 청크는 OpenAI \texttt{text-embedding-3-small}로
1{,}536차원 벡터화되어 ChromaDB 컬렉션 \texttt{jeju\_folklore\_chunks}에 저장된다.
검색 시점에서 사용자가 도착한 장소명(예: ``삼도동'')이 같은 임베딩 공간으로
변환되어, ``각시당본풀이'' 청크 0과의 코사인 거리가 약 0.4로 측정되면
\texttt{distance $<$ 0.75} 임계값을 통과해 페르소나 프롬프트의 컨텍스트로
주입된다. 본 시스템은 이러한 가공·검색 경로를 비짓제주 관광지와 직접 매핑된
397 설화·민담에 적용해 \textbf{1{,}262 청크}를 동행자 채팅의 의미 검색 대상으로
한정한다.
...
\subsection{코스 점수 계산의 실 코스 예시}

각 후보 코스에 대한 점수는 다음 수식으로 계산된다.

\begin{center}
\(\mathrm{score} = \big(\sum_c \mathrm{user\_score}[c] \times \mathrm{share}[c]\big) \times 10\)
\end{center}

\noindent 여기서 $c$는 5종 설화 카테고리, $\mathrm{user\_score}[c]$는 사용자
입력 점수(1순위 3점, 2순위 1점, 나머지 0점), $\mathrm{share}[c]$는 코스의
매핑 중 카테고리 $c$의 점유율이다. specificity $\ge 5$ 매핑만 사용하며,
가중치 $w = \mathrm{specificity}/5.0$로 정확한 지명일수록 점수에 큰 영향을
준다.

\textbf{[예시]} 코스 ``조금 이른 여름휴가''(서부, 2일)의 매핑은 8건이다.
애월해안도로에 4건(금덕본향·비매닛할망당·상가본향·알당), 사려니숲 에코힐링에
4건(도체비 심엉·꿩빙에기·주막동산 도체빗불·한흘과 논흘)이 매핑되어 있다.
카테고리별 분포는 생활민담·교훈담 37.5\%, 무속신화·신격 25\%, 마을 공동체
12.5\%, 해양·어촌 12.5\%, 초자연 12.5\%이다. 사용자가 1순위로
생활민담·교훈담(3점), 2순위로 무속신화·신격(1점)을 선택한 경우
$\mathrm{score} = (3 \times 0.375) + (1 \times 0.25) + 0 = 1.375$이며,
스케일링하여 $\mathbf{13.75}$\textbf{점}이 된다. 이 점수가 양수이므로 추천
풀에 포함되어 \texttt{random.sample(3)}에서 선택될 가능성을 갖는다. 동일
방식으로 1{,}090개 큐레이션 코스 각각에 점수가 부여되고, score $> 0$인
후보만 풀에 남아 무작위 3건이 사용자에게 노출된다.
...
(1) \textbf{ChromaDB 의미 검색}: 모든 설화·민담 505건의 청크를 ChromaDB에 저장하되,
비짓제주 관광지와 직접 매핑된 397건(\textbf{1{,}262 청크})에만 \texttt{is\_mapped=True}
메타데이터를 부여하고 동행자 채팅 검색 쿼리에 \texttt{where} 필터를 적용해
\emph{매핑된 설화 청크만} 검색 대상으로 제한한다. (2) \textbf{사전 매핑 테이블}: 매핑 출처를 \texttt{source} 컬럼
...
\textbf{\texttt{has\_gps} $\to$ \texttt{is\_mapped} 전환 경위와 효과.}\quad
초기 ChromaDB 구현은 GPS 좌표 부착 여부(\texttt{has\_gps} 메타데이터)로 검색
대상을 제한하였다(228건, 961 청크). 그러나 본 서비스의 핵심 가치는 ``이
장소의 설화를 들려준다''이며, GPS 좌표가 있다는 사실이 ``비짓제주 관광지와
의미적으로 연결된다''를 보장하지 않는다. 이에 메타데이터를 \texttt{is\_mapped}로
전환하여, 사전 매핑 테이블에 실제 등장한 397 설화·민담의 청크 1{,}262개만
검색 대상으로 제한하였다(SSoT: \texttt{place\_folklore\_mapping}에서
\texttt{source} $\ne$ \texttt{'gps\_assist'} 기준 distinct
\texttt{folklore\_code\_no}).

이 전환의 효과는 두 가지다. 첫째, 검색 대상 청크가 961 → 1{,}262로 약 31\%
증가하여 동행자 채팅의 콘텐츠 풍부도가 향상되었다. 둘째, ``GPS가 있다''는
우회 기준이 아닌 ``관광지와 직접 매핑된다''는 서비스 본질적 기준으로
일관성이 확보되었다. 사전 매핑 테이블의 \texttt{source} 필터(\texttt{WHERE
source != 'gps\_assist'})와 동일한 설계 패턴이 양 검색 경로에 적용된다.
...
TourAPI\,[9]에서 사진·정보를 7일 캐시 기준으로 조회한다.
...
민화 이미지 프롬프트를 동시에 받는다. 이미지 프롬프트는 gpt-image-1\,[12]에
...
\textbf{다단계 함수 파이프라인}으로 설화 배경 장소를 먼저 추출하고 Visit Jeju로
동선을 보완하는 방식이었으나, 설화 위치 분포가 희박할 때 코스가 비현실적으로
...
ChromaDB 의미 검색 대상 청크 & \textbf{1{,}262개} \\
...
\textbf{(1) 페르소나 응답 품질의 정량 평가 부재.}\quad 5종 페르소나의 말투
차별화는 시스템 프롬프트와 퓨샷 예시에 의존하지만, 동일 설화에 대한 5종
응답의 의미적 다양성과 캐릭터 일관성을 정량적으로 평가하지 못하였다.
BLEU·BERTScore 등 자동 지표나 사용자 평가 실험을 통해 페르소나 분리 설계의
효과를 수치로 검증하는 작업이 향후 필요하다.

\textbf{(2) 청크 분할 전략의 단순성.}\quad 현재는 900자 + 120 overlap의
슬라이딩 윈도우로 분할하나, 설화 본문의 의미 단위(서사 단락·인물 등장 등)를
반영하지 않는다. 의미 단위 청크화나 LLM 기반 청크 요약을 적용하면 검색
정밀도가 향상될 가능성이 있다.

\textbf{(3) 실사용자 검증 데이터 부재.}\quad 모든 실험은 시뮬레이션
환경에서 수행되었으며, 실제 제주 방문자의 GPS 도착 감지·동행자
채팅·일지 생성에 대한 사용성 평가가 부재하다. 특히 제주 현지의 다중
경로·도심 협곡 환경에서의 지오펜스 신뢰성, 페르소나 적합성, 일지 만족도는
시뮬레이션만으로는 검증할 수 없다. App Store 출시 이후 수집될 사용자 행동
데이터로 본 시스템의 가설을 검증할 예정이다.

\textbf{(4) 공공데이터 미매핑 108건의 활용.}\quad 505건 공공데이터 중
비짓제주 관광지와 매핑되지 않은 108건은 현재 ChromaDB에 저장되어 있으나
사용자 노출에서는 제외된다. 본문이 매우 짧거나 매칭 가능 지명이 없는
경우로, 향후 자유 발화 챗봇이나 위치 무관 추천에서 활용 방안을 검토한다.
...
\textbf{2단계 결정론적 코스 추천 함수 파이프라인}, 5종 페르소나 LLM
...
사용한다. 설화·민담 505건은 ChromaDB에 벡터화하되, 비짓제주 관광지와 직접
매핑된 397 설화·민담의 \textbf{1{,}262 청크}만 동행자 채팅의 의미 검색 대상으로 한정한다.
