...
|---|---|---|
| `production` | nolmeongbopseo-production.up.railway.app | 앱스토어 iOS 1.0 (2) — **공모전 심사(2026-10 말)까지 다시 배포하지 않는다** |
| `apps-in-toss` | nolmeongbopseo-apps-in-toss.up.railway.app | 이 미니앱 (`src/api/config.ts`) |

- 둘 다 같은 `../backend/` 코드다. 출시본 코드는 git 태그 `ios-1.0-build2` 에 있다.
- 서버를 고친 뒤 배포는 `railway up --detach --environment apps-in-toss --service nolmeongbopseo` (저장소 루트에서). **`--environment` 를 빼지 않는다** — 빠뜨리고 연결이 production 이면 심사 중인 앱의 서버가 바뀐다.
