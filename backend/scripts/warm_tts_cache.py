...
## 왜 10곳만인가 (2026-08-20 조익준님 결정)

189곳을 다 만들 필요가 없다. 데모와 심사에서 보여줄 것만 있으면 된다.
목소리도 아직 확정이 아니라, 지금 다 만들면 목소리를 바꿀 때 전부 버린다.

대상은 `data/tts_warm_top10.json`이고 **비짓제주 코스 9,134개의 등장 빈도**로
뽑았다(공항·카페 등 제외). stid를 손으로 바꾸면 이 스크립트가 그대로 따라간다.

## 주의

- 무료 플랜은 월 15,000 크레딧이고 10곳이 약 9,800이다. 한 번은 되지만
  **목소리를 바꿔 다시 만들면 한 달 안에 두 번은 안 된다.**
...
TARGETS = BASE / "data" / "tts_warm_top10.json"
...
def main() -> int:
    ...
    places = json.loads(TARGETS.read_text(encoding="utf-8"))
    ...
    for p in places:
        ...
        try:
            ...
            story = get_story(request=None, stid=str(p["stid"]), lang=args.lang)
